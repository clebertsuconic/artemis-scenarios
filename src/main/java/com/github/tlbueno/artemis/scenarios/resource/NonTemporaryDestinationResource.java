...
@Log4j2
@Path("/nonTemporaryDestination")
@ToString(callSuper = true)
public class NonTemporaryDestinationResource extends BaseResource {
    @POST
    @Consumes(APPLICATION_YAML)
    @Produces(APPLICATION_YAML)
    public Response createScenario(NonTemporaryDestinationScenarioRequest scenarioRequest) {
        ...
        ScenarioResponse scenarioResponse = ScenarioResponse.builder()
                .withId(id)
                .build();
        Scenario<NonTemporaryDestinationScenarioRequest> scenario = Scenario.<NonTemporaryDestinationScenarioRequest>builder()
                .id(id)
